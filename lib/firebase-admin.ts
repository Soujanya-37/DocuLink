import fs from "node:fs";
import path from "node:path";
import { cert, getApps, initializeApp } from "firebase-admin/app";
import { getAuth } from "firebase-admin/auth";
import { getFirestore } from "firebase-admin/firestore";

// ---- Interface for type safety ----
interface ServiceAccountKey {
  type: string;
  project_id: string;
  private_key_id: string;
  private_key: string;
  client_email: string;
  client_id: string;
  auth_uri: string;
  token_uri: string;
  auth_provider_x509_cert_url: string;
  client_x509_cert_url: string;
  universe_domain: string;
}

// ---- Normalize newlines in private key ----
function normalizePrivateKey(obj: any) {
  if (obj && typeof obj.private_key === "string") {
    obj.private_key = obj.private_key.replace(/\\n/g, "\n");
  }
  return obj;
}

// ---- Try parsing plain JSON string ----
function tryParseJson(raw: string) {
  try {
    const parsed = JSON.parse(raw);
    return normalizePrivateKey(parsed);
  } catch {
    return undefined;
  }
}

// ---- Try decoding base64-encoded key ----
function tryParseBase64(raw: string) {
  try {
    const decoded = Buffer.from(raw, "base64").toString("utf8");
    const parsed = JSON.parse(decoded);
    return normalizePrivateKey(parsed);
  } catch {
    return undefined;
  }
}

// ---- Try loading JSON file from disk ----
function tryReadJsonFile(filePath: string) {
  try {
    const fullPath = path.isAbsolute(filePath)
      ? filePath
      : path.join(process.cwd(), filePath);
    if (!fs.existsSync(fullPath)) return undefined;
    const content = fs.readFileSync(fullPath, "utf8");
    const parsed = JSON.parse(content);
    return normalizePrivateKey(parsed);
  } catch {
    return undefined;
  }
}

// ---- Core resolver (clean version) ----
function resolveServiceAccount(): ServiceAccountKey | undefined {
  // 1️⃣ From direct JSON string in env
  const rawKey = process.env.FIREBASE_SERVICE_ACCOUNT_KEY;
  if (rawKey) {
    const asJson = tryParseJson(rawKey);
    if (asJson) return asJson;
    const asB64 = tryParseBase64(rawKey);
    if (asB64) return asB64;
  }

  // 2️⃣ From Base64 variable
  const keyB64 = process.env.FIREBASE_SERVICE_ACCOUNT_KEY_BASE64;
  if (keyB64) {
    const parsed = tryParseBase64(keyB64);
    if (parsed) return parsed;
  }

  // 3️⃣ From file path in env
  const keyFile =
    process.env.FIREBASE_SERVICE_ACCOUNT_KEY_FILE ||
    process.env.FIREBASE_SERVICE_ACCOUNT_KEY_PATH ||
    process.env.GOOGLE_APPLICATION_CREDENTIALS;

  if (keyFile) {
    const parsed = tryReadJsonFile(keyFile);
    if (parsed) return parsed;
  }

  // 4️⃣ Fallback: look for common local filenames
  for (const candidate of [
    "serviceAccount.json",
    "service-account.json",
    "firebase-service-account.json",
  ]) {
    const parsed = tryReadJsonFile(candidate);
    if (parsed) return parsed;
  }

  // ❌ Nothing found
  console.warn(
    "[firebase-admin] No valid service account credentials found. Server-side Firebase features will fail."
  );
  return undefined;
}

// ---- Initialize Firebase Admin ----
const serviceAccount = resolveServiceAccount();

const app =
  getApps().length === 0
    ? initializeApp({
        credential: serviceAccount ? cert(serviceAccount as any) : undefined,
        projectId:
          serviceAccount?.project_id ||
          process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID,
      })
    : getApps()[0];

export const adminAuth = getAuth(app);
export const adminDb = getFirestore(app);
export default app;
